...
def lnlike_moffat3_s(theta, spec, specE, x_t, y_t, db_m, dx, dy, e_m, tht_m, al_m, beta, ellip, alpha):
    model=mod.moffat_model3_s(theta, x_t=x_t, y_t=y_t, db_m=db_m, dx=dx, dy=dy, e_m=e_m, tht_m=tht_m, al_m=al_m, beta=beta, ellip=ellip, alpha=alpha)
    LnLike = -0.5*np.nansum(((spec-model)/specE)**2.0)#/np.float(len(theta))
    return LnLike

def lnlike_moffat3(theta, spec, specE, x_t, y_t, db_m, bn, ns, e_m, tht_m, ellip, dxt, dyt, fcenter, re_int, Re_c):
    model=mod.moffat_model3(theta, x_t=x_t, y_t=y_t, be_t=db_m, bn=bn, ns=ns, e_m=e_m, tht_m=tht_m, ellip=ellip, dxt=dxt, dyt=dyt, fcenter=fcenter, re_int=re_int, Re_c=Re_c)
    LnLike = -0.5*np.nansum(((spec-model)/specE)**2.0)#/np.float(len(theta))
    return LnLike
...
def lnlike_moffat0(theta, spec, specE, x_t, y_t, db_m, e_m, tht_m, ellip):
    model=mod.moffat_model0(theta, x_t=x_t, y_t=y_t, be_t=db_m, e_m=e_m, tht_m=tht_m, ellip=ellip)
    LnLike = -0.5*np.nansum(((spec-model)/specE)**2.0)#/np.float(len(theta))
    return LnLike
...
def lnprior_mofft3_s(theta, At1=20, beta=True, ellip=False, alpha=True):
    if beta:
        if ellip:
            At,ds_t,db_t,e_t,tht_t=theta
            if ((ds_t >= 5)) and ((db_t >= 0.5)) and ((At >= 0) and (At < At1)) and ((e_t >= 0) and (e_t < 1))  and ((tht_t >= 0.0) and (tht_t < 180.0)):# and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                return 0.0
            else:
                return -np.inf
        else:
            ...
            # and (ds_t <= 152)
            # and (db_t <= 162)
            if ((ds_t >= 5)) and ((db_t >= 0.5)) and ((At >= 0) and (At < At1)):# and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                return 0.0
            else:
                return -np.inf
    else:
        if ellip:
            ...
            if ((ds_t >= 5)) and ((At >= 0) and (At < At1)) and ((e_t >= 0) and (e_t < 1)) and ((tht_t >= 0.0) and (tht_t < 180.)):# and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                return 0.0
            else:
                return -np.inf
        else:
            if alpha:
                At,ds_t=theta
                # and (ds_t <= 152) and (At < At1)
                if ((ds_t >= 5)) and ((At >= 0)):# and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                    return 0.0
                else:
                    return -np.inf
            else:
                At=theta
                # and (ds_t <= 152) and (At < At1)
                if ((At >= 0)):# and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                    return 0.0
                else:
                    return -np.inf

def lnprior_mofft3(theta, At1=20, ellip=False, fcenter=False, re_int=False):
    if ellip:
        if fcenter:
            if re_int:
                At,Io,ds_t,e,th0=theta
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and (Io >= 0) and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 < 180)):                 
                    return 0.0
                else:
                    return -np.inf
            else:
                At,Io,Re,ds_t,e,th0=theta
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)) and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 < 180)):                 
                    return 0.0
                else:
                    return -np.inf
        else:
            if re_int:
                ...
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and ((dx >= -5) and (dx <= 5)) and ((dy >= -5) and (dy <= 5)) and (Io >= 0) and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 < 180)):                 
                    return 0.0
                else:
                    return -np.inf
            else:
                At,dx,dy,Io,Re,ds_t,e,th0=theta
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and ((dx >= -5) and (dx <= 5)) and ((dy >= -5) and (dy <= 5)) and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)) and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 < 180)):                 
                    return 0.0
                else:
                    return -np.inf
    else:
        if fcenter:
            if re_int:
                At,Io,ds_t=theta#,e,th0
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and (Io >= 0):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                    return 0.0
                else:
                    return -np.inf
            else:
                At,Io,Re,ds_t=theta#,e,th0
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                    return 0.0
                else:
                    return -np.inf
        else:
            if re_int:
                At,dx,dy,Io,ds_t=theta#,e,th0
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and ((dx >= -5) and (dx <= 5)) and ((dy >= -5) and (dy <= 5)) and (Io >= 0):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                    return 0.0
                else:
                    return -np.inf
            else:
                At,dx,dy,Io,Re,ds_t=theta#,e,th0
                if ((ds_t >= 10) and (ds_t <= 22)) and ((At >= 0) and (At < At1)) and ((dx >= -5) and (dx <= 5)) and ((dy >= -5) and (dy <= 5)) and (Io >= 0) and ((Re >= 0.5) and (Re <= 50.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
                    return 0.0
                else:
                    return -np.inf
...
def lnprior_mofft0(theta, At1=20, ellip=False):
    if ellip:
        ...
        #Ao=Io*np.exp(bn)
        if ((ds_t >= 2) and (ds_t <= 22)) and ((At >= 0) and (At < (At1*20))) and ((dx >= -5) and (dx <= 5)) and ((dy >= -5) and (dy <= 5)) and (Io >= 0) and ((bn >= 0.5) and (bn <= 50.0)) and ((Re >= 0.5) and (Re <= 50.0)) and ((ns >= 0.5) and (ns <= 2.0)):#10.0# and ((e >= 0.0) and (e <= 10.0)) and ((th0 >= 0) and (th0 <= 2.0*np.pi)):                 
            return 0.0
        else:
            return -np.inf
...
def lnprob_moffat3_s(theta, spec, specE, x_t, y_t, db_m, At, dx, dy, e_t, tht_t, beta, ellip):
    lp = lnprior_mofft3_s(theta, At1=At, beta=beta, ellip=ellip)#alpha=alpha
    if not np.isfinite(lp):
        return -np.inf
    else:
        return lp + lnlike_moffat3_s(theta, spec, specE, x_t, y_t, db_m, dx, dy, e_t, tht_t, 5.0, beta, ellip, True) #alpha

def lnprob_moffat3(theta, spec, specE, x_t, y_t, db_m, At, bn, ns, e_t, tht_t, ellip, Re_c, re_int, dxt, dyt, fcenter):
    lp = lnprior_mofft3(theta, At1=At, ellip=ellip, fcenter=fcenter, re_int=re_int)
    if not np.isfinite(lp):
        return -np.inf
    else:
        return lp + lnlike_moffat3(theta, spec, specE, x_t, y_t, db_m, bn, ns, e_t, tht_t, ellip, dxt, dyt, fcenter, re_int, Re_c) 
...
def lnprob_moffat0(theta, spec, specE, x_t, y_t, db_m, At, e_t, tht_t, ellip):
    lp = lnprior_mofft0(theta, At1=At, ellip=ellip)
    if not np.isfinite(lp):
        return -np.inf
    else:
        return lp + lnlike_moffat0(theta, spec, specE, x_t, y_t, db_m, e_t, tht_t, ellip) 
