...
def evaluate_2dPSF(pf_map,pf_mapE,name='test',Model_name='moffat',Usermods=['','',''],Labelvalues=[],Namevalues=[],Inpvalues=[],Infvalues=[],Supvalues=[],path_out='',savefig=True,autocent=True,logP=True,sig=2,plot_f=False,stl=False,smoth=True,sig=1.8,ncpu=10,valsI={}):
    if plot_f:
        tim=True
    else:
        tim=False
    ...
    if plot_f:
        try:
            ...
            spec_hst=spec_agn*0
        tol.plot_models_maps(pf_map,spec_agn,spec_hst,samples,name=name,path_out=path_out,savefig=savefig,Labelvalues=Labelvalues,logP=logP,stl=stl,smoth=smoth,sig=sig)
