...
def map_ana(map,mapE,hdr,sig=2,prior_config='priors_prop.yaml',prior_pathconf='',Usermods=['extern','pathext','extern_function.py'],mod_ind=0,mod_ind0=0,verbose=False,dir_o='',name='spectra',ncpu=10,logP=True,stl=False):
    ...
    pars_max,psf1,Ft,FtF=evaluate_2dPSF(map,mapE,name=name,Usermods=Usermods,Model_name=Model_name,Labelvalues=Labelvalues,Namevalues=Namevalues,Inpvalues=Inpvalues,Infvalues=Infvalues,Supvalues=Supvalues,sig=sig,plot_f=True,ncpu=ncpu,valsI=valsI,path_out=dir_o,logP=logP,stl=stl)
