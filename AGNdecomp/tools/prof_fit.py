...
def prof_ana(cube,cubeE,hdr,sig=2,prior_config='priors_prop.yaml',prior_pathconf='',Usermods=['extern','pathext','extern_function.py'],wavew1=4850,wavew2=5150,mod_ind=0,mod_ind0=0,verbose=False,psamp=10,tp='',dir_o='',name='spectra',str_p=False,local=False,ncpu=10,sp=0,logP=True,smoth=True,sigm=1.8,ofsval=-1,stl=False,psfmod=False,psfmodData=None):
    ...
    if local == False:
        ...
        for i in range(0, nz_t):
            ...
            pars_max,psf1,Ft,FtF=evaluate_2dPSF(map1,map1e,name=name+spt,Usermods=Usermods,Model_name=Model_name,Labelvalues=Labelvalues,Namevalues=Namevalues,Inpvalues=Inpvalues,Infvalues=Infvalues,Supvalues=Supvalues,sig=sig,ncpu=ncpu,valsI=valsI,logP=logP,stl=stl,smoth=smoth,sigm=sigm,ofsval=ofsval,path_out=dir_o,psfmod=psfmod,psfmodData=psfmodData)
